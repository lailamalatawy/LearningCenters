...
<?php 
    ...
    die("Could not connect:"  . mysqli_connect_error()); } 
      ...
      $output = "";
    $result = mysqli_query($conn, "SELECT activity_id, activity_name, activity_date, location FROM Activities WHERE school_id = 1 AND administrator_id = 16;
") or die('cannot show tables');    // CHANGE SQL QUERY ENTRIES!
    $val = 1;     
    while ($row = mysqli_fetch_array($result)){
      $output .= '
      <tr>
      <td> <form  action ="admin_action.php" method="post">
           <input type="hidden" name ="la_acid' . $val . '"></input>
           </form>  '. $row['activity_id'] . '
      </td>
      <td>' . $row['activity_name'] . '</td> 
      <td>' . $row['activity_date'] . '</td>                                                                                   
      <td>' . $row['location'] . '</td> 
      <td> <form  action ="admin_action.php" method="post">
           <input type="text" name ="la_tidActivity' . $val . '" class="input" maxlength="20" style = "border:1px solid #000000; width: 100px; height: 20px"> 
           </form> 
      </td>       
      <td> <form  action ="admin_action.php" method="post">
           <span><span><input type="submit" name ="la_assignTeachAct' . $val . '"  class="button" value = "Assign"></input></span></span>
           </form> 
      </td>
      </tr>';
      $val = $val+1;
    ...
    <section id="content">
      <div class="box1">
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
              <h2 class="pad_bot1">Assign Activities to Teachers</h2>
            </div>
            <div class="pad_left1">
