...
<html lang="en">
<head>
<title>Learn Center | My Courses</title>
<meta charset="utf-8">
    ...
    <section id="content">
      <div class="box1">
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
              <h2 class="pad_bot1">Courses</h2>
            </div>
            <?php             $count = '';
            $result = mysqli_query($conn, "call view_Courses_per_Teacher('$id')"); 
                if (!$result) { die("Query to show fields from table failed"); }
                while($row = mysqli_fetch_row($result)){
                  if($row[3] != $count){
                    $count = $row[3];
                    echo '<div class="wrapper pad_bot2">';
                    echo '<div id="h_font1">'.$row[3].' Level'.'</div><br>';
                  }
                  echo '<div class="wrapper pad_bot2">';
                  echo '<figure class="left marg_right1"><img src="images/course.png" alt=""></figure>';
                  echo '<p class="pad_bot1 pad_top2"><strong>'.$row[0].'</strong>  '.$row[1].'</p>';
                  echo '<strong>Level: </strong>'.$row[3].'    <strong>Grade: </strong> '.$row[4].'<br>';
               }
      ?>
          </article>
