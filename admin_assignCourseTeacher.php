...
<?php 
    ...
    die("Could not connect:"  . mysqli_connect_error()); } 
      ...
      $output = "";
    $result = mysqli_query($conn, "SELECT course_code, course_name, level_name, grade FROM Courses;") or die('cannot show tables');    // CHANGE SQL QUERY ENTRIES!
    $val = 1;     
    while ($row = mysqli_fetch_array($result)){
      $output .= '
      <tr>
      <td> <form  action ="admin_action.php" method="post">
           <input type="hidden" name ="la_ccode' . $val . '"></input>
           </form>  '. $row['course_code'] . '
      </td>
      <td>' . $row['course_name'] . '</td> 
      <td>' . $row['level_name'] . '</td>                                                                                   
      <td>' . $row['grade'] . '</td> 
      <td> <form  action ="admin_action.php" method="post">
           <input type="text" name ="la_tidCourse' . $val . '" class="input" maxlength="20" style = "border:1px solid #000000; width: 100px; height: 20px"> 
           </form> 
      </td>       
      <td> <form  action ="admin_action.php" method="post">
           <span><span><input type="submit" name ="la_assignTeachCourse' . $val . '"  class="button" value = "Assign"></input></span></span>
           </form> 
      </td>
      </tr>';
      $val = $val+1;
    ...
    <section id="content">
      <div class="box1">
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
              <h2 class="pad_bot1">Assign Teachers to Courses</h2>
            </div>
            <div class="pad_left1">
