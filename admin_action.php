...
{	$atitle = $_POST["la_atitle"];
    ...
    // $adminid="1";
     ...
     if (!mysqli_query($conn, "CALL add_Announcement2('$atitle', '$atype', '$adate', '$adesc','$username');")) {
		// echo $username;
		header('Location: admin_postNewAnnouncement.php?login='.$username.'@'.$schoolid); 	
	}
	else{
...
{	$acname = $_POST["la_acname"];
	...
	if (!mysqli_query($conn, "CALL create_Activity2('$actype', '$acdate', '$acloc', '$adesc','$acname','$schoolid' ,'$username');")) {
		// echo $username;
