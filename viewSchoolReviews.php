    ...
    <section id="content">
      <div class="box1">
        <div class="wrapper">
          <article class="col1">
            <div class="pad_left1">
              <h2 class="pad_bot1">Your School Reviews</h2>
            </div>
            
           <?php
 
              
                   $result = mysqli_query($conn, "select s.name, p.review_txt,s.school_id From Schools s INNER JOIN Parent_review_School p ON s.school_id=p.school_id and p.parent_username ='$username' ");               
                   
                if (!$result) { die("Query to show fields from table failed");  }
                while($row = mysqli_fetch_row($result)){
                echo '<div class="wrapper pad_bot2">';
                  //echo '<figure class="left marg_right1"><img src="images/school.jpg" alt=""></figure>';
                  echo '<p class="pad_bot1 pad_top2">You Reviewed : <strong>'.$row[0].' </strong>as being : <br><strong>"'.$row[1].'"</strong></p>';
